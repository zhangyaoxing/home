...
class MetroLine(Horizontal):
    ...
    def refresh_data(self):
        entry = self.entry
        advertised_time = datetime.fromisoformat(entry["AdvertisedTimeAtLocation"])
        now = datetime.now(tz=pytz.UTC)
        track = entry.get("TrackAtLocation", "")
        line = entry.get("Line", "") or ""
        dest = "/".join(
            f"[bold][green]{self.stations.get(loc, loc)}[/green][/bold]"
            for loc in entry.get("ToLocation", [])
        )
        delta = int((advertised_time - now).total_seconds() / 60)

        self.query_one(".schedule-route", Static).update(f"{line} {dest}")
        self.query_one(".schedule-track", Static).update(track)
        self.query_one(".schedule-time", Static).update(f"{delta} min")
...
class MetroEntry(Static):
    ...
    def compose(self) -> ComposeResult:
        yield MetroLine(self.entry, self.stations)
        tr_map = self.entry.get("Deviation_tr", {})
        messages = []
        for raw_msg in _as_list(self.entry.get("Deviation")):
            normalized = _normalize_message(raw_msg)
            if not normalized:
                continue
            display = tr_map.get(normalized, normalized)
...
class MetroSchedule(Static):
    ...
    def _check_cache(self):
        ...
        data = cached["data"]
        self.stations = data.get("station_names", {})
        station = self.stations.get(config["sl"]["metroStationCode"], "")
        self.border_subtitle = f"{station}  [dim]Updated {format_cache_time(cached)}[/]"

        entries = data.get("announcements", [])
        now = datetime.now(tz=pytz.UTC)
        self.remove_children()
        self.mount(
            Horizontal(
        ...
        for entry in entries:
            advertised_time = datetime.fromisoformat(
                entry["AdvertisedTimeAtLocation"]
            )
            if now > advertised_time:
                continue
            self.mount(MetroEntry(entry, self.stations))
